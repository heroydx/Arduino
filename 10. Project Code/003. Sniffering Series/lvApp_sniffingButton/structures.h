...
#define CONST_ETH_MAC_LEN 6
#define CONST_SSID_LEN 33
...
struct beaconinfo
{
  uint8_t bssid[CONST_ETH_MAC_LEN];
  uint8_t ssid[CONST_SSID_LEN];
  //int ssid_len;
  uint8_t ssid_len;
  uint8_t channel;
  uint8_t err;
...
{
  uint8_t bssid[CONST_ETH_MAC_LEN];
  uint8_t station[CONST_ETH_MAC_LEN];
  uint8_t ap[CONST_ETH_MAC_LEN];
  uint8_t channel;
  uint8_t err;
  uint16_t seq_n;
  signed rssi;
...
struct clientinfo parse_data(uint8_t *frame, uint16_t framelen, signed rssi, unsigned channel)
{
  ...
  memcpy(ci.station, station, CONST_ETH_MAC_LEN);
  memcpy(ci.bssid, bssid, CONST_ETH_MAC_LEN);
  memcpy(ci.ap, ap, CONST_ETH_MAC_LEN);
...
struct beaconinfo parse_beacon(uint8_t *frame, uint16_t framelen, signed rssi)
{
    ...
    while (pos < framelen) {
      switch (frame[pos]) {
        case 0x00: //SSID
          ...
          bi.ssid_len = (uint8_t) frame[pos + 1];
          if (bi.ssid_len == 0) {
        ...
        case 0x03: //Channel
          ...
          bi.channel = (uint8_t) frame[pos + 2];
          pos = -1;
          break;
        default:
          break;
  ...
  bi.capa[0] = frame[34];
  bi.capa[1] = frame[35];
  memcpy(bi.bssid, frame + 10, CONST_ETH_MAC_LEN);
  return bi;
}
