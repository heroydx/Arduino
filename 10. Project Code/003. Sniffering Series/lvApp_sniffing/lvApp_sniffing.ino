...
#define CONST_DEVICE_VERSION "1_23_20170215"
/* 软件版本解释：
...
*/
#define CONST_DEVICE_SWVN "LVSW_GATE_A_S01_V1.0.8_20170814"
#define CONST_DEVICE_HWVN "LVHW_GATE_4M1M_V1.0.0_20161116"
#define CONST_DEVICE_MAID "1"
#define CONST_DEVICE_PID "23"
...
#define CONST_CHANNEL_SWITCH_DELAY 20
...
bool bSend_3001()
{
  ...
  /*
    ...
    for (u = 0; u < aps_known_count; u++)
    {
      ...
      {
        memcpy(homeMAC, aps_known[u].bssid, CONST_ETH_MAC_LEN);
        homeChannel = aps_known[u].channel;
        break;
      }
    }
  ...
  */
  ...
  if (gnKnowCount > 0)
  {
      ...
      *spCurr++ = ',';
      for (u = 0; u < aps_known_count; u++)
      {
        if (! memcmp(aps_known[u].bssid, clients_known[i].bssid, CONST_ETH_MAC_LEN))
        {
          ...
          {
            //if (!memcmp(clients_known[i].station, homeMAC, CONST_ETH_MAC_LEN))
      ...
      //sprintf(spCurr, "\"name\":\"\"");
      //spCurr += strlen(spCurr);
      sprintf(spCurr, "\"stamp\":\"%d\"", clients_known[i].timeStamp);
      spCurr += strlen(spCurr);
      *(spCurr++) = '}';
      *(spCurr++) = ',';
    }
    ...
    *(spCurr++) = 0;
    ...
    //LVDATA.gCommandBuff.chpData[0] = 0;
    //strcpy(LVDATA.gCommandBuff.chpData, spStart);
    //DBGPRINTF("\n%s\n", LVDATA.gCommandBuff.chpData);
    ...
    //LVDATA.gnServerCommand=CONST_CMD_3001;
