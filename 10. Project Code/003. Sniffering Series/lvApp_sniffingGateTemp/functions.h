...
#define MAX_CLIENTS_TRACKED 240
#define CONST_MIN_SIGNAL -70
//#define CONST_MIN_SIGNAL -100

uint8_t homeSSID[CONST_SSID_LEN];
uint8_t homeMAC[CONST_ETH_MAC_LEN];
unsigned int homeChannel = 1;
signed minSingalThreshold = CONST_MIN_SIGNAL;
  ...
  else
  {
    ...
    if ((sniffer->buf[0] == 0x08) || (sniffer->buf[0] == 0x88))
    {
      ...
      if (memcmp(ci.bssid, ci.station, CONST_ETH_MAC_LEN))
      {
        ...
        */
        //仅判断是在一个AP的设备.否则都按照-70dbm过滤
        if (!memcmp(ci.bssid, homeMAC, CONST_ETH_MAC_LEN)) {
          isHomeDevice = 1;
        }
        //modified on 2017/6/22 end
