...
//#define CONST_AIRKISS_DEVICE_TYPE     "gh_9e2cff3dfa51" //wechat public number
//#define CONST_AIRKISS_DEVICE_ID       "122475" //model ID

#define CONST_AIRKISS_DEVICE_TYPE     "chakonger" //wechat public number
...
airkiss_wifilan_time_callback(void)
{
  ...
  if (ret != AIRKISS_LAN_PAKE_READY) {
    Serial.printf("Pack lan packet error!");
    return;
  }

  ret = espconn_sendto(&pssdpudpconn, lan_buf, lan_buf_len);
  if (ret != 0) {
    Serial.printf("UDP send error!");
  }
  Serial.printf("Finish send notify!\n");
}
...
airkiss_wifilan_recv_callbk(void *arg, char *pdata, unsigned short len)
{
  ...
  Serial.printf("\n\r== airkiss_wifilan_recv_callbk Begin==\n\r");
  Serial.printf("\n\r%s", pdata);
  ...
  switch (ret) {
    case AIRKISS_LAN_SSDP_REQ:
      ...
      packret = airkiss_lan_pack(AIRKISS_LAN_SSDP_RESP_CMD,
                                 (void *)deviceType, (void *) deviceID, IMEI, IMEILEN, lan_buf, &lan_buf_len, &akconf);
      ...
      for (i = 0; i < lan_buf_len; i++)
        Serial.printf("%c", lan_buf[i]);
      Serial.printf("\r\n\r\n");

      packret = espconn_sendto(&pssdpudpconn, lan_buf, lan_buf_len);
      if (packret != 0) {
        Serial.printf("LAN UDP Send err!");
      }
      bAirKissDoneFlag = true;
      break;
    default:
      Serial.printf("Pack is not ssdq req!%d\r\n", ret);
      break;
  }
  Serial.printf("\n\r== airkiss_wifilan_recv_callbk End==\n\r");
}
...
void loop()
{
  ...
  {
    Serial.println("airkiss_wifilan_time_callback");
    Serial.println(bAirKissDoneFlag);
