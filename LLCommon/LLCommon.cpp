...
//Modified by Steven Lian on 2017/09/05
/*
...
static void private_filter_server_command()
{
  ...
  for (i = 0; i < nLen; i++)
  {
    ...
    if (cmd == CONST_CMD_2001) {
      if ((filterData.command != CONST_CMD_2001) || (filterData.beginTick != 0) || (filterData.endTick != 0)) {
        //if ((filterData.command != CONST_CMD_2001)) {
      ...
      //if (filterData.command == CONST_CMD_2001 && filterData.beginTick==0) {
      if (filterData.command == CONST_CMD_2001 ) {
        //first 3001
        cmd = CONST_CMD_2001;
      }
...
void ICACHE_FLASH_ATTR LLCommon::vInit_common_data(int nType)
{
  ...
  bKeepConnection = true; //default = true;
  ...
  sprintf(gachServerSeed,"%5d",random(10000, 60000));
  ...
  //encode/decode
  //strncpy(codingType,CONST_DEVICE_CODING_TYPE,_DEF_COMM_CODING_TYPE_LENGTH);
}
...
bool ICACHE_FLASH_ATTR LLCommon::bSend_registration(bool firstReg, char achSeed[])
{
  return bSend_registration(IMEI, firstReg, achSeed);
}


bool ICACHE_FLASH_ATTR LLCommon::bSend_registration(char achReqID[], bool firstReg, char achSeed[])
{
  int ret;
  byte  *bssid;
  //byte mac[6];
  char chReg;
  DBGPRINTLN("--send 1001--");
  ...
  //         CONST_CMD_1001, IMEI, gsDeviceInfo.Version, gsDeviceInfo.MAID, gsDeviceInfo.PID, bssid[0], bssid[1], bssid[2], bssid[3], bssid[4], bssid[5], chReg, gsDeviceInfo.SWVN, gsDeviceInfo.HWVN,
  ...
  sprintf (gBigBuff.chpData, "{\"CMD\":\"%d\",\"reqID\":\"%s\",\"rtnID\":\"%s\",\"SN\":\"%d\",\"MAID\":\"%s\",\"PID\":\"%s\",\"WifiMac\":\"%02X-%02X-%02X-%02X-%02X-%02X\",\"regType\":\"%c\",\"ROMVersion\":\"%s\",\"HWVersion\":\"%s\",\"regMac\":\"%02X-%02X-%02X-%02X-%02X-%02X\",\"bootReason\":\"%d\",\"seed\":\"%s\"}",
           CONST_CMD_1001, achReqID, IMEI, SN, gsDeviceInfo.MAID, gsDeviceInfo.PID, bssid[0], bssid[1], bssid[2], bssid[3], bssid[4], bssid[5], chReg, gsDeviceInfo.SWVN, gsDeviceInfo.HWVN,
  ...
  DBGPRINTLN("debug send 1001 gBigBuff.achData:");
  DBGPRINTLN(gBigBuff.chpData);

  encode_data(codingType, strlen(gBigBuff.chpData), gBigBuff.chpData, gCommandBuff.chpData);

  DBGPRINTLN(gCommandBuff.chpData);

  ret = nCommonPOST(gBigBuff.chpData, false);
...
short  ICACHE_FLASH_ATTR LLCommon::decode_data(char achData[], char achBuff[])
{
  short ret;
  char *spCodingType, *spData;
  short nCodingTypeLen, nDataLen;
  short nLen;

  nCodingTypeLen = nGet_json_pos(achData, "codingType", &spCodingType);
  nDataLen = nGet_json_pos(achData, "data", &spData);

  if (nCodingTypeLen > 0 && nDataLen > 0) {

    //valid data
    *(spCodingType + nCodingTypeLen) = '\0';
    *(spData + nDataLen) = '\0';

    nLen = strlen(spData);
    
    //DBGPRINTF("\ndecode_data:[%s] [%s]",spCodingType,spData);

    if (!strncmp(spCodingType, _DEF_COMM_CODING_TYPE_BCD, _DEF_COMM_CODING_TYPE_LENGTH)) {
      //BCD
      ret = bcd_decode((unsigned char *)achBuff, (unsigned char *)spData, nLen);
      //DBGPRINTF("\n codingType:[%s],[%d],[%s]",spCodingType,nLen,achBuff);
    }
    else if (!strncmp(spCodingType, _DEF_COMM_CODING_TYPE_B6400, _DEF_COMM_CODING_TYPE_LENGTH)) {
      //standard base64
      ret = Base64decode(achBuff, spData);
      //DBGPRINTF("\n codingType:[%s],[%d],[%s]",spCodingType,nLen,achBuff);
    }
    else if (!strncmp(spCodingType, _DEF_COMM_CODING_TYPE_B6401, _DEF_COMM_CODING_TYPE_LENGTH)) {
      ret = simpleB64_decode((char *)achBuff, (char *)spData, nLen);
      //DBGPRINTF("\n codingType:[%s],[%d],[%s]",spCodingType,nLen,achBuff);
      //for (short i=0;i<nLen;i++){
      //  DBGPRINTF("%02X ",achData[i]);
      //};

    }
    else if (!strncmp(spCodingType, _DEF_COMM_CODING_TYPE_B6402, _DEF_COMM_CODING_TYPE_LENGTH)) {
      ret = simpleB64_decode((char *)achBuff, (char *)spData, nLen);
      //DBGPRINTF("\n codingType:[%s],[%d],[%s]",spCodingType,nLen,achBuff);

    }
    else if (!strncmp(spCodingType, _DEF_COMM_CODING_TYPE_B64C1, _DEF_COMM_CODING_TYPE_LENGTH)) {
      ret = simpleB64_decode((char *)achBuff, (char *)spData, nLen);
      //DBGPRINTF("\n codingType:[%s],[%d],[%s]",spCodingType,nLen,achBuff);

    }
    else {
      //default  _DEF_COMM_CODING_TYPE_JSON
      strcpy(achBuff, spData);
      ret = nLen;
      //DBGPRINTF("\n codingType default:[%s],[%s]",spCodingType,achBuff);
    }
    strcpy(achData, achBuff);
  }
...
short ICACHE_FLASH_ATTR LLCommon::encode_data(char *codingType, short nLen, char achData[], char achBuff[])
{
  short ret;
  char *spCodingType;
  spCodingType = codingType;

  if (!strncmp(spCodingType, _DEF_COMM_CODING_TYPE_BCD, _DEF_COMM_CODING_TYPE_LENGTH)) {
    //BCD
    ret = bcd_encode((unsigned char *)achBuff, (unsigned char *)achData, nLen);
    sprintf(achData, "{\"codingType\":\"%s\",\"data\":\"%s\"}", spCodingType, achBuff);
  }
  else if (!strncmp(spCodingType, _DEF_COMM_CODING_TYPE_B6400, _DEF_COMM_CODING_TYPE_LENGTH)) {
    //standard base64
    ret = Base64encode(achBuff, achData, nLen);
    sprintf(achData, "{\"codingType\":\"%s\",\"data\":\"%s\"}", spCodingType, achBuff);
  }
  else if (!strncmp(spCodingType, _DEF_COMM_CODING_TYPE_B6401, _DEF_COMM_CODING_TYPE_LENGTH)) {
    ret = simpleB64_encode((char *)achBuff, (char *)achData, nLen);
    sprintf(achData, "{\"codingType\":\"%s\",\"data\":\"%s\"}", spCodingType, achBuff);
  }
  else if (!strncmp(spCodingType, _DEF_COMM_CODING_TYPE_B6402, _DEF_COMM_CODING_TYPE_LENGTH)) {
    ret = simpleB64_encode((char *)achBuff, (char *)achData, nLen);
...
bool ICACHE_FLASH_ATTR LLCommon::bHandle_recvLine_body(char achData[])
{
  //push message
  if (memcmp(achData, CONST_CMD_2001_STR, CONST_CMD_LENGTH) == 0)
  {
    DBGPRINTLN("==get push message==");
    //gnServerCommand = CONST_CMD_3001;
    push_cmd(CONST_CMD_2001);
  ...
  else if (achData[0] == '{')
  {
    ...
    char *spCMD, *spDEVID, *spDevTodo, *spActionID, *spParam, *spInst, *spInterval, *spRtnID, *spRTN;
    short nCMDLen, nDEVIDLen, nDevTodoLen, nActionIDLen, nParamLen, nInstLen, nIntervalLen, nRtnIDLen, nRTNLen;
    char *spPos;
    short nT1;
    //int nTEMPCount=0;
    ...
    //decode, due to  bHandle_recvLine_main(&gCommandBuff.chpData[posStart]);
    //user gCommandBuff,so, we will use gBigBuff as decode buff
    nT1=decode_data(achData, gBigBuff.chpData);
    DBGPRINTLN("\n after decode:");
    DBGPRINTLN(achData);
    if (nT1<=0){
      //decoding error,return 
      DBGPRINTF("\n decoding error !!! [%d]",nT1);
      return false;
    }
    ...
    nDEVIDLen = nGet_json_pos(achData, "reqID", &spDEVID);
    nRtnIDLen = nGet_json_pos(achData, "rtnID", &spRtnID);
     
    if (strncmp(IMEI, spDEVID, IMEILen) || (IMEILen!=nDEVIDLen)) {
      //reqID != rtn ID or the command is not for this device
      DBGPRINTF("\n Not for this device:IMEI [%s]",IMEI);
      nExec_transfer_Command(achData);
    }
    else {
      // command for this device
      DBGPRINTF("\n for this device:IMEI [%s] -- CMD [%d]",IMEI,nCMDVal);

      switch (nCMDVal)
      {
        case CONST_CMD_1001:
          nRTNLen = nGet_json_pos(achData, "RTN", &spRTN);
          if (nRTNLen>0){
            short rtn;
            *(spRTN + nRTNLen) = '\0';
            rtn=atoi(spRTN);
...
bool ICACHE_FLASH_ATTR LLCommon::bHandle_recvLine_date(char achData[])
{
...
bool ICACHE_FLASH_ATTR LLCommon::bHandle_recvLine_main(char achData[])
{
  bool ret = true;
...
void ICACHE_FLASH_ATTR LLCommon::push_cmd(short cmd, short beginTime, short endTime)
{
  short pos;
...
void ICACHE_FLASH_ATTR LLCommon::push_cmd(short cmd)
{
  push_cmd(cmd, 0, 0);
}


short ICACHE_FLASH_ATTR LLCommon::pop_cmd()
{
  short ret = 0;
  short pos;
...
void ICACHE_FLASH_ATTR LLCommon::vRefresh_data_status()
{
  /*
...
bool ICACHE_FLASH_ATTR LLCommon::func_cmd_901(char  achParam[], char achInst[])
{
  bool ret = true;
...
bool ICACHE_FLASH_ATTR LLCommon::func_cmd_904(char  achParam[], char achInst[])
{
  bool ret = true;
  push_cmd(CONST_CMD_9999);
...
bool ICACHE_FLASH_ATTR LLCommon::func_cmd_905(char  achParam[], char achInst[])
{
  bool ret = false;
  int pos, pos2, nLen;
  char *spStart, *spEnd;
  ...
  *(spEnd) = '\0';
  ...
  gchMainState = WAIT_CONNECTION_STATE;
  //gchMainState = WAIT_FOR_THIRD_PARTY_DEVICE;
...
bool ICACHE_FLASH_ATTR LLCommon::bLoad_HWVersion()
{
  bool bRet = false;
  int nLen;
...
bool ICACHE_FLASH_ATTR LLCommon::bSave_HWVersion()
{
...
bool ICACHE_FLASH_ATTR LLCommon::bEncode(char *achpData)
{
  ...
  if (nLen > (CONST_BIG_BUFF_SIZE - 100))
    return false;
  /*
  // first char
  ...
  source[4] = 0;
  simple_encode_data((unsigned char *) encodeData, (unsigned char *)source, 5);
  */
  
  //gTime.timeString
  sprintf(spSprintPos, ",\"YMDHMS\":\"%s\"}", gTime.timeString);
...
void ICACHE_FLASH_ATTR LLCommon::vSystem_restart(short reason)
{
...
void LLCommon::mainsetup()
{
  ...
  version = CONST_LLCOMMON_VERSION;
  DBGPRINTLN();
...
unsigned short LLCommon::get_version()
{
  return version;
}


void LLCommon::func_INIT_STATE()
{
...
void LLCommon::func_SMARTCONFIG_START_STATE()
{
  ...
  else
  {
    if (ulGet_interval(glStateTransferTicks) >= CONST_TRANSFER_TICK_LEN)
    {
      ...
      if (gnStateTransferTicksCount > CONST_SMATCONFIG_REBOOT_COUNT) {
        // to reboot device to re-scan wifi network;
        DBGPRINTLN("\n smartconfig reboot to scan new wifi network.");
        if (bKeepConnection)
...
void LLCommon::func_SMARTCONFIG_DONE_STATE()
{
  //show_led_color(2, 2);
...
void LLCommon::func_LL_CONFIG_START_STATE()
{
      ...
      *(spSeed + nSeedLen) = '\0';
      ...
      if ((nLoRaAddrLen > 0) && (nLoRaPortLen > 0) && (nLoRaChannelLen > 0)) {
        ...
        *(spLoRaChannel + nLoRaChannelLen) = '\0';
        ...
        gsDeviceInfo.server.loraChannel = atoi(spLoRaChannel);
        
        //temp solution,暂时没有配置软件的办法
        //strncpy(gsDeviceInfo.server.addr, spLoRaAddr, CONST_ADDR_LENGTH);
      }
    }
    else
    {
...
void LLCommon::func_LL_CONFIG_DONE_STATE()
{
  LEDControl.change_LED_status(CONST_LED_STATUS_CONNECTING);
  ...
  DBGPRINTF("SVR:%s\r\n", gsDeviceInfo.server.addr);
  DBGPRINTF("SPT:%d\r\n", gsDeviceInfo.server.port);
  DBGPRINTF("LORA Channel:%d\r\n", gsDeviceInfo.server.loraChannel);
  ...
  delay(50);
  bSend_registration(true, gachServerSeed);
...
void LLCommon::func_WAIT_FOR_THIRD_PARTY_DEVICE()
{
  //DBGPRINTLN("WAIT FOR THIRD PARTY DEVICE:");
  //LoRa etc device setup
  vThird_party_setup_call();
  gchMainState=CONNECTED_STATE;
}

void LLCommon::func_WAIT_CONNECTION_STATE()
{
  ...
  {
    //Connected
    LEDControl.change_LED_status(CONST_LED_STATUS_SERVER);
    bConnectWiFi = false;
    ...
    {
      if (gbConfigOK)
        bSend_registration(false, gachServerSeed);
      else
        bSend_registration(true, gachServerSeed);
      gu8PowerOn = CONST_POWERON_FLAG_NO;
    }
    ...
    LEDControl.change_LED_status(CONST_LED_STATUS_ALWAYSOFF);
  }
...
void LLCommon::func_CONNECTED_STATE()
{
  ...
  {
    //gnServerCommand = CONST_CMD_2001;
    push_cmd(CONST_CMD_2001);
  ...
  {
    DBGPRINTLN("\nclean gnTcpCommandBusy flag");
    gnTcpCommandBusy = 0;
    //nDoubleCheckTCPFlagCount=0;
  }
...
void LLCommon::func_FACTORY_TEST_FINISH_LOOP()
{
...
void LLCommon::debug_print_info()
{
  /*
    DBGPRINT("DATE:");
...
void LLCommon::func_100ms_loop()
{
  // LED Control
  LEDControl.display_LED_color();
}

void LLCommon::func_second_loop()
{
  //force post check
  ulForcePOSTCheckTimeInMs = MIN(CONST_FORCE_POST_CHECK_TIME_TICK_LENGTH, glConnIntervalTick * CONST_FORCE_POST_CHECK_TIME_TICK_COUNT);
...
void LLCommon::func_minute_loop()
{
  //reboot
...
void LLCommon::mainloop()
{
  ...
  switch (gchMainState)
  {
    ...
    case LV_CONFIG_START_STATE:
      func_LL_CONFIG_START_STATE();
      break;

    case LV_CONFIG_DONE_STATE:
      func_LL_CONFIG_DONE_STATE();
      break;
    ...
    case WAIT_FOR_THIRD_PARTY_DEVICE:
      func_WAIT_FOR_THIRD_PARTY_DEVICE();
      break;
